...
@section('title')
    ...
    Add connection to {{ $participant->id_code }}

@endsection

@section('list')

    <h3 class="list-header">Add connection to {{ $participant->id_code }}</h3>
    <form method='POST' action='/connection/saveConnection/{{ $participant->id }}'>
        {{ csrf_field() }}
        <table>
            <tr>
